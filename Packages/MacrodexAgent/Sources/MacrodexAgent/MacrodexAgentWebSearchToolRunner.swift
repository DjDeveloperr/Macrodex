...
public struct MacrodexAgentWebSearchResult: Codable, Equatable, Sendable, Identifiable {
...
public protocol MacrodexAgentWebSearchHTTPTransport {
    func perform(_ request: URLRequest) throws -> (Data, HTTPURLResponse)
}

public enum MacrodexAgentWebSearchToolError: Error, Equatable, LocalizedError {
    case missingQuery
    case invalidURL
    case httpStatus(Int)
...
public final class MacrodexAgentWebSearchToolRunner: MacrodexAgentToolRunner {
    private let transport: any MacrodexAgentWebSearchHTTPTransport
    private let timeout: TimeInterval

    public init(
        transport: any MacrodexAgentWebSearchHTTPTransport = URLSessionMacrodexAgentWebSearchHTTPTransport(),
    ...
    public func runTool(_ call: MacrodexAgentToolCall) throws -> MacrodexAgentToolResult {
        ...
        guard !query.isEmpty else {
            return Self.failureResult(callID: call.id, query: nil, error: MacrodexAgentWebSearchToolError.missingQuery)
        }
        let maxResults = max(3, min(args["maxResults"]?.intValue ?? 5, 10))
        let results: [MacrodexAgentWebSearchResult]
        do {
    ...
    private static func failureResult(callID: String, query: String?, error: Error) -> MacrodexAgentToolResult {
    ...
    public func search(query: String, maxResults: Int = 5) throws -> [MacrodexAgentWebSearchResult] {
        var components = URLComponents(string: "https://html.duckduckgo.com/html/")
        components?.queryItems = [
            URLQueryItem(name: "q", value: query)
        ]
        guard let url = components?.url else {
            throw MacrodexAgentWebSearchToolError.invalidURL
        }

        var request = URLRequest(url: url, timeoutInterval: timeout)
        request.httpMethod = "GET"
        request.setValue("MacrodexAgent/0.1", forHTTPHeaderField: "User-Agent")
        let (data, response) = try transport.perform(request)
        guard (200...299).contains(response.statusCode) else {
            throw MacrodexAgentWebSearchToolError.httpStatus(response.statusCode)
        }
        guard let html = String(data: data, encoding: .utf8) else {
            throw MacrodexAgentWebSearchToolError.invalidResponse
        }
        return Self.parseDuckDuckGoHTML(html, maxResults: maxResults)
    }

    public static func parseDuckDuckGoHTML(_ html: String, maxResults: Int) -> [MacrodexAgentWebSearchResult] {
        ...
        var results: [MacrodexAgentWebSearchResult] = []
        ...
        for block in blocks {
            ...
            results.append(MacrodexAgentWebSearchResult(title: title, url: url, snippet: snippet))
        }
        return results
    }

    private static func parseDuckDuckGoResultLinks(_ html: String, maxResults: Int) -> [MacrodexAgentWebSearchResult] {
        ...
        for match in matches {
            ...
            results.append(MacrodexAgentWebSearchResult(title: title, url: url))
        }
        return results
    }
...
public struct URLSessionMacrodexAgentWebSearchHTTPTransport: MacrodexAgentWebSearchHTTPTransport {
    public init() {}
    ...
    public func perform(_ request: URLRequest) throws -> (Data, HTTPURLResponse) {
        ...
        let task = session.dataTask(with: request) { data, response, error in
            ...
            guard let httpResponse = response as? HTTPURLResponse else {
                result = .failure(MacrodexAgentWebSearchToolError.invalidResponse)
                return
            }
