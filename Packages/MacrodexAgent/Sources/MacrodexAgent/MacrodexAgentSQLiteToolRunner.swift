...
public enum MacrodexAgentSQLiteToolError: Error, Equatable, LocalizedError {
...
public struct MacrodexAgentSQLiteTransactionOperation: Equatable, Sendable {
    public var purpose: String?
    public var statement: String
    public var bindings: [MacrodexAgentJSONValue]
    public var mode: MacrodexAgentSQLiteToolRunner.Mode

    public init(
        purpose: String? = nil,
        statement: String,
        bindings: [MacrodexAgentJSONValue] = [],
        mode: MacrodexAgentSQLiteToolRunner.Mode = .auto
    ) {
...
public final class MacrodexAgentSQLiteToolRunner: MacrodexAgentToolRunner {
    ...
    public func runTool(_ call: MacrodexAgentToolCall) throws -> MacrodexAgentToolResult {
        ...
        do {
            ...
            case .schema:
                ...
                output.objectValueAssign("purpose", value: purpose.map(MacrodexAgentJSONValue.string) ?? .null)
                return MacrodexAgentToolResult(callID: call.id, output: output)
            case .validate:
                guard !statement.isEmpty else {
                    return Self.failureResult(callID: call.id, error: MacrodexAgentSQLiteToolError.missingStatement, statement: statement, mode: mode)
                }
    ...
    public func query(_ statement: String, bindings: [MacrodexAgentJSONValue] = []) throws -> [MacrodexAgentJSONValue] {
        try validateLeadingComment(statement)
    ...
    @discardableResult
    public func exec(_ statement: String, bindings: [MacrodexAgentJSONValue] = []) throws -> Int {
        try validateLeadingComment(statement)
        return try withDatabase { db in
    ...
    public func validate(_ statement: String, bindings: [MacrodexAgentJSONValue] = []) throws -> MacrodexAgentJSONValue {
        try validateLeadingComment(statement)
        return try withDatabase { db in
    ...
    public func schema(tables requestedTables: [String] = []) throws -> MacrodexAgentJSONValue {
        try withDatabase { db in
            let tableNames = try resolvedSchemaTables(requestedTables, db: db)
            let tables = try tableNames.map { tableName -> MacrodexAgentJSONValue in
    ...
    public func transaction(
        _ operations: [MacrodexAgentSQLiteTransactionOperation],
        dryRun: Bool = false
    ) throws -> MacrodexAgentJSONValue {
        guard !operations.isEmpty else {
            throw MacrodexAgentSQLiteToolError.missingOperations
        }
        ...
        return try withDatabase { db in
            ...
            var results: [MacrodexAgentJSONValue] = []
            do {
                for (index, operation) in operations.enumerated() {
                    ...
                    case .query:
                        ...
                        results.append([
                            ...
                            "purpose": operation.purpose.map(MacrodexAgentJSONValue.string) ?? .null,
                            "mode": "query",
    ...
    public static func transactionOperations(from value: MacrodexAgentJSONValue?) throws -> [MacrodexAgentSQLiteTransactionOperation] {
        guard let array = value?.arrayValue else {
            throw MacrodexAgentSQLiteToolError.invalidOperations
        }
        return try array.map { item in
            ...
            return MacrodexAgentSQLiteTransactionOperation(
                purpose: purpose,
                statement: statement,
                bindings: bindings,
                mode: mode
    ...
    private func withDatabase<T>(_ body: (OpaquePointer) throws -> T) throws -> T {
        ...
        guard sqlite3_open(databaseURL.path, &db) == SQLITE_OK else {
            ...
            throw MacrodexAgentSQLiteToolError.sqlite(message)
        }
        guard let db else {
            throw MacrodexAgentSQLiteToolError.databaseNotOpen
        }
        defer {
            sqlite3_close(db)
        }
        ...
        guard sqlite3_exec(db, "PRAGMA foreign_keys = ON; PRAGMA busy_timeout = 5000;", nil, nil, nil) == SQLITE_OK else {
            throw MacrodexAgentSQLiteToolError.sqlite(Self.lastError(db))
        }
        return try body(db)
    }

    private func query(_ statement: String, bindings: [MacrodexAgentJSONValue], db: OpaquePointer) throws -> [MacrodexAgentJSONValue] {
        var prepared: OpaquePointer?
        guard sqlite3_prepare_v2(db, statement, -1, &prepared, nil) == SQLITE_OK else {
            throw MacrodexAgentSQLiteToolError.sqlite(Self.lastError(db))
        }
        guard let prepared else {
            throw MacrodexAgentSQLiteToolError.sqlite("SQLite did not return a prepared statement.")
        }
        defer {
        ...
        var rows: [MacrodexAgentJSONValue] = []
    ...
    @discardableResult
    private func exec(_ statement: String, bindings: [MacrodexAgentJSONValue], db: OpaquePointer) throws -> Int {
    ...
    private func validate(_ statement: String, bindings: [MacrodexAgentJSONValue], db: OpaquePointer) throws -> MacrodexAgentJSONValue {
        var prepared: OpaquePointer?
        guard sqlite3_prepare_v2(db, statement, -1, &prepared, nil) == SQLITE_OK else {
            throw MacrodexAgentSQLiteToolError.sqlite(Self.lastError(db))
        }
    ...
    private func validateLeadingComment(_ statement: String) throws {
        ...
        guard let comment = Self.leadingSQLComment(statement) else {
            throw MacrodexAgentSQLiteToolError.missingLeadingComment
        }
        if let requiredLeadingCommentMarker,
           !comment.localizedCaseInsensitiveContains(requiredLeadingCommentMarker) {
            throw MacrodexAgentSQLiteToolError.missingRequiredCommentMarker(requiredLeadingCommentMarker)
        }
    }

    private static func bindings(from value: MacrodexAgentJSONValue?) throws -> [MacrodexAgentJSONValue] {
        guard let value else {
            return []
        }
        guard let array = value.arrayValue else {
            throw MacrodexAgentSQLiteToolError.invalidBindings
        }
        return array
    }

    private static func stringArray(from value: MacrodexAgentJSONValue?) -> [String] {
        value?.arrayValue?.compactMap(\.stringValue) ?? []
    }

    private static func failureResult(callID: String, error: Error, statement: String? = nil, mode: Mode? = nil) -> MacrodexAgentToolResult {
    ...
    private static func errorType(for error: Error) -> String {
        if case MacrodexAgentSQLiteToolError.sqlite = error {
            return "sqlite"
        }
        if let toolError = error as? MacrodexAgentSQLiteToolError {
            switch toolError {
    ...
    private static func hint(for error: Error, statement: String?) -> String {
        if case MacrodexAgentSQLiteToolError.missingLeadingComment = error {
            return "Start the SQL text with a /* macrodex: Label */ or -- macrodex: Label comment."
        }
        if case MacrodexAgentSQLiteToolError.missingRequiredCommentMarker(let marker) = error {
            return "Put \(marker) in the first SQL comment so the UI can label the tool call."
        }
        if case MacrodexAgentSQLiteToolError.invalidBindings = error {
    ...
    private static func bind(_ values: [MacrodexAgentJSONValue], to statement: OpaquePointer) throws {
        for (index, value) in values.enumerated() {
            ...
            guard result == SQLITE_OK else {
                throw MacrodexAgentSQLiteToolError.sqlite("Failed to bind SQL value at position \(position).")
            }
        }
    }

    private static func row(from statement: OpaquePointer) -> MacrodexAgentJSONValue {
        let columnCount = sqlite3_column_count(statement)
        var object: [String: MacrodexAgentJSONValue] = [:]
    ...
    private static func value(from statement: OpaquePointer, column index: Int32) -> MacrodexAgentJSONValue {
...
private extension MacrodexAgentJSONValue {
    mutating func objectValueAssign(_ key: String, value: MacrodexAgentJSONValue) {
