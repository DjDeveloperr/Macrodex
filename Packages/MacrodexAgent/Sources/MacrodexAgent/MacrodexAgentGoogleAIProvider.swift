...
public final class MacrodexAgentGoogleAIProvider: MacrodexAgentProviderClient {
    ...
    public init(
        apiKey: String,
        baseURL: URL = MacrodexAgentGoogleAIProvider.defaultBaseURL,
    ...
    public func complete(_ request: MacrodexAgentProviderRequest) throws -> MacrodexAgentProviderResponse {
        ...
        urlRequest.setValue("MacrodexAgent/1.0", forHTTPHeaderField: "User-Agent")

        let (data, response) = try transport.perform(urlRequest)
        guard (200...299).contains(response.statusCode) else {
            throw MacrodexAgentGoogleAIProviderError.httpStatus(response.statusCode, Self.safeResponseSnippet(data))
        }
        return try parseGenerateContentResponse(data)
    }

    private func makeGenerateContentBody(_ request: MacrodexAgentProviderRequest) throws -> Data {
        ...
        guard JSONSerialization.isValidJSONObject(body) else {
            throw MacrodexAgentGoogleAIProviderError.invalidRequestBody("Gemini payload is not JSON-serializable.")
        }
        return try JSONSerialization.data(withJSONObject: body)
    }

    private func makeContent(_ message: MacrodexAgentMessage, toolNamesByCallID: [String: String]) throws -> [String: Any]? {
        switch message.role {
        case .system:
            return nil
        case .user:
    ...
    private func makeFunctionDeclaration(_ tool: MacrodexAgentToolDefinition) throws -> [String: Any] {
        [
            "name": tool.name,
    ...
    private func parseGenerateContentResponse(_ data: Data) throws -> MacrodexAgentProviderResponse {
        guard let object = try JSONSerialization.jsonObject(with: data) as? [String: Any] else {
            throw MacrodexAgentGoogleAIProviderError.invalidResponse("Expected a JSON object.")
        }
        if let error = object["error"] as? [String: Any] {
            throw MacrodexAgentGoogleAIProviderError.invalidResponse(Self.errorMessage(error))
        }
        ...
        let message = text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
            ? nil
            : MacrodexAgentMessage(role: .assistant, content: text)
        if message == nil && toolCalls.isEmpty {
            throw MacrodexAgentGoogleAIProviderError.noAssistantOutput
        }
        return MacrodexAgentProviderResponse(message: message, toolCalls: toolCalls, usage: usage)
    }

    private func sanitizedSchemaObject(_ value: MacrodexAgentJSONValue) throws -> Any {
    ...
    private static func toolCall(from part: [String: Any]) throws -> MacrodexAgentToolCall? {
        guard let functionCall = part["functionCall"] as? [String: Any] else {
            return nil
        }
        let name = functionCall["name"] as? String ?? ""
        let args = functionCall["args"] ?? [:]
        return MacrodexAgentToolCall(
            id: UUID().uuidString.lowercased(),
            name: name,
            arguments: try MacrodexAgentJSONValue(jsonObject: args)
        )
    }
...
public enum MacrodexAgentGoogleAIProviderError: Error, Equatable, LocalizedError {
