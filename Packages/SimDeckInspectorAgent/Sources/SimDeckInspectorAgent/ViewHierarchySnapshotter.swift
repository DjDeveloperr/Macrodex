...
final class ViewHierarchySnapshotter {
    ...
    func node(for view: UIView, includeHidden: Bool, maxDepth: Int?, depth: Int) -> InspectorViewNode {
        ...
        let displayName = Self.displayName(for: className)
        return InspectorViewNode(
            id: Self.id(for: view),
            className: className,
            displayName: displayName,
            moduleName: Self.moduleName(for: displayName ?? className),
            ...
            isOpaque: swiftUIBridgeClassName(className) ? false : view.isOpaque,
            ...
            swiftUI: swiftUIInfo(for: view, className: className, displayName: displayName),
    ...
    private func swiftUIInfo(for view: UIView, className: String, displayName: String?) -> InspectorSwiftUIInfo? {
        let payload = view.simDeckInspectorTagPayload
        let normalizedName = displayName ?? className
        let isHost = className.contains("SwiftUI")
            || className.contains("UIHosting")
            || normalizedName.contains("SwiftUI")
            || normalizedName.contains("UIHosting")
    ...
    private func swiftUIMetadata(
        ...
        if let displayName, displayName != className {
            metadata["displayName"] = displayName
        }
        if swiftUIBridgeClassName(displayName ?? className) {
            metadata["bridge"] = "true"
            metadata["subviewCount"] = String(subviewCount)
        }
        return metadata
    }
    ...
    private static func displayName(for className: String) -> String? {
        let demangled = SwiftNameDemangler.demangle(className) ?? className
        let simplified = simplifySwiftRuntimeName(demangled)
        guard simplified != className else {
            return nil
        }
        return simplified
    }

    private static func simplifySwiftRuntimeName(_ name: String) -> String {
        var value = name
        if let privateMarker = value.range(of: #"\.\(unknown context at \$[0-9a-fA-F]+\)\."#, options: .regularExpression) {
            value.replaceSubrange(privateMarker, with: ".")
        }
        value = value.replacingOccurrences(of: "SwiftUI.", with: "")
        value = value.replacingOccurrences(of: "UIKit.", with: "")
        value = value.replacingOccurrences(of: "SimDeckInspectorAgent.", with: "")
        value = value.replacingOccurrences(of: "Macrodex.", with: "")

        if value.contains("NavigationStackHostingController") && value.contains("HostingView") {
            return "NavigationStack Hosting View"
        }
        if value.contains("UIKitPlatformViewHost") && value.contains("NavigationStackRepresentable") {
            return "NavigationStack Bridge"
        }
        if value.contains("UIKitPlatformViewHost") && value.contains("SimDeckInspectorTagRepresentable") {
            return "SwiftUI Inspector Tag Bridge"
        }
        if value.contains("UIKitPlatformViewHost") && value.contains("SimDeckSwiftUIViewTreePublisherRepresentable") {
            return "SwiftUI Tree Publisher Bridge"
        }
        if value.contains("UIKitPlatformViewHost") && value.contains("KeyboardDismissTapInstaller") {
            return "Keyboard Dismiss Bridge"
        }
        if value.contains("UIKitPlatformViewHost") && value.contains("DrawerPanGestureInstaller") {
            return "Drawer Gesture Bridge"
        }
        if value.contains("UIKitPlatformViewHost") && value.contains("ConversationComposerTextView") {
            return "Composer Text View Bridge"
        }
        if value.contains("UIKitPlatformViewHost") && value.contains("UIKitPagingView") {
            return "SwiftUI Paging Bridge"
        }
        if value.contains("PagingCollectionView") {
            return "Paging Collection View"
        }
        if value.contains("UIKitPagingCell") {
            return "Paging Cell"
        }
        if value.contains("UIKitBarItemHost") {
            return "Navigation Bar Item Host"
        }
        if value.contains("_UIInteractionView") {
            return "Interaction View"
        }

        let genericReplacements: [(String, String)] = [
            (#"UIHostingView<ModifiedContent<AnyView, RootModifier>>"#, "SwiftUI Root Host"),
            (#"UIKitPlatformViewHost<PlatformViewControllerRepresentableAdaptor<NavigationStackRepresentable>>"#, "NavigationStack Bridge"),
            (#"UIKitPlatformViewHost<PlatformViewRepresentableAdaptor<SimDeckInspectorTagRepresentable>>"#, "SwiftUI Inspector Tag Bridge"),
            (#"UIKitPlatformViewHost<PlatformViewRepresentableAdaptor<UIKitPagingView>>"#, "SwiftUI Paging Bridge"),
            (#"NavigationStackHostingController<AnyView>.HostingView"#, "NavigationStack Hosting View"),
            (#"HostingScrollView.PlatformGroupContainer"#, "Scroll Content Group"),
            (#"HostingScrollView.PlatformContainer"#, "Scroll Container"),
            (#"HostingScrollView"#, "Scroll View"),
            (#"_UIInheritedView"#, "Environment Bridge"),
            (#"UIKitNavigationBar"#, "Navigation Bar"),
        ]

        for (pattern, replacement) in genericReplacements where value.contains(pattern) {
            return replacement
        }

        if value.contains("UIKitPlatformViewHost<PlatformViewRepresentableAdaptor<") {
            return "SwiftUI UIKit View Bridge"
        }
        if value.contains("PlatformViewControllerRepresentableAdaptor<") {
            return "SwiftUI View Controller Bridge"
        }
        if value.contains("FloatingBarHostingView") {
            return "Floating Bar Host"
        }
        if value.contains("UIPlatformGlassInteractionView") {
            return "Glass Interaction View"
        }
...
private func swiftUIBridgeClassName(_ className: String) -> Bool {
    className.contains("SwiftUI")
        || className.contains("UIHosting")
        || className.contains("NavigationStackHostingController")
        || className.contains("HostingScrollView")
        || className.contains("UIKitPlatformViewHost")
}

enum SwiftNameDemangler {
    static func demangle(_ name: String) -> String? {
        guard name.hasPrefix("_Tt") || name.hasPrefix("$s") else {
            return nil
        }
        return name.withCString { pointer in
            let length = UInt(strlen(pointer))
            guard let demangled = swift_demangle(pointer, length, nil, nil, 0) else {
                return nil
            }
