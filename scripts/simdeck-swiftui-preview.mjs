...
function xcodeSwiftcArgs(xcode) {
  ...
  pushSearchArgs(args, "-I", [
    ...
    settings.GENERATED_MODULEMAP_DIR,
  ]);
  pushModuleMapArgs(args, [
    settings.GENERATED_MODULEMAP_DIR,
    xcodeSourcePackageCheckouts(settings),
  ]);
...
function xcodeSourcePackageCheckouts(settings) {
  if (settings.DERIVED_DATA_DIR) {
    return path.join(settings.DERIVED_DATA_DIR, "SourcePackages", "checkouts");
  }
...
function pushModuleMapArgs(args, roots) {
  for (const root of roots) {
    if (!root || !fs.existsSync(root)) {
      continue;
    }
    const stats = fs.statSync(root);
    const moduleMaps = stats.isDirectory()
      ? [...walkFiles(root)].filter((file) => file.endsWith(".modulemap"))
      : [];
    for (const moduleMap of moduleMaps) {
      args.push(
        "-I",
        path.dirname(moduleMap),
        "-Xcc",
...
function sanitizePreviewSource(source, moduleName, reexportModule = false) {
  const stripped = source.replace(
    /(^|\n)(\s*)@main\b/g,
    "$1$2// @main stripped by SimDeck preview runner",
  );
  return xcodeImportPrelude(moduleName, reexportModule) + stripped;
}

function xcodeImportPrelude(moduleName, reexportModule = false) {
  if (!moduleName) {
    return "";
  }
  const exported = reexportModule ? "@_exported " : "";
  return `#if DEBUG
${exported}@testable import ${moduleName}
#else
${exported}import ${moduleName}
#endif
