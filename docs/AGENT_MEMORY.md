...
- FoundationModels support must use only public SDK symbols. Keep the on-device `system` model as the no-login default when available; expose `pcc` only as an unavailable/public-SDK-gated option until Apple exposes a public PCC selector or entitlement.
