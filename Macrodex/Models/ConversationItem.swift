...
struct ConversationDynamicToolCallData: Equatable {
    ...
    var isHiddenMetadataTool: Bool {
        tool.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "title"
    }
}
...
struct ConversationItem: Identifiable, Equatable {
    ...
    var isVisuallyEmptyNeutralItem: Bool {
        ...
        case .dynamicToolCall(let data):
            return data.isHiddenMetadataTool
        default:
            return false
        }
    }
...
private extension HydratedConversationItemContent {
    func conversationItemContent(itemId: String) -> ConversationItemContent {
        ...
        case .widget(let data):
            ...
            let status = data.status.trimmingCharacters(in: .whitespacesAndNewlines)
            return .note(
                ConversationNoteData(
                    title: title.isEmpty ? "Rich content unavailable" : title,
                    body: status.isEmpty ? "This response included rich content that Macrodex does not render." : status
                )
            )
        case .userInputResponse(let data):
