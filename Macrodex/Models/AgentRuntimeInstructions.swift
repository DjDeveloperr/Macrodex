...
enum AgentRuntimeInstructions {
    ...
    - If the user gives at least some nutrition data, such as calories, estimate missing protein/carbs/fat instead of leaving them at zero. Use the food type and calorie balance to make a reasonable estimate, and mention when values are estimated.
    ...
    - Do not web-search common generic foods when local food memory or built-in estimates are good enough, including simple items like eggs, banana, oatmeal, chicken breast, rice, coffee, and milk.
