...
final class AppLifecycleController {
    static let notificationServerIdKey = "macrodex.notification.serverId"
    static let notificationThreadIdKey = "macrodex.notification.threadId"
    ...
    func handleBackgroundPush(appModel: AppModel) async {
        let signpostID = OSSignpostID(log: appLifecycleSignpostLog)
    ...
    private func reloadTrackedThread(appModel: AppModel, key: ThreadKey) async {
        ...
        let config = AppThreadLaunchConfig(
            ...
            approvalPolicy: nil,
            sandbox: nil,
            developerInstructions: AgentRuntimeInstructions.developerInstructions(for: key),
    ...
    private func shouldTrustLiveThreadState(
        ...
        // Always refresh; the local store can no-op if live data is already fresh.
        false
    }
