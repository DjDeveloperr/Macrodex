...
enum MacrodexAgentHealthKitToolError: Error, LocalizedError {
    case missingCommand
...
final class MacrodexAgentHealthKitToolRunner: MacrodexAgentToolRunner {
    ...
    func runTool(_ call: MacrodexAgentToolCall) throws -> MacrodexAgentToolResult {
        ...
        guard !normalized.isEmpty else {
            return Self.failureResult(callID: call.id, error: MacrodexAgentHealthKitToolError.missingCommand)
        }
        ...
        if exitCode != 0 {
            return Self.failureResult(callID: call.id, error: MacrodexAgentHealthKitToolError.commandFailed(exitCode, output))
        }

        if let data = output.data(using: .utf8),
           let object = try? JSONSerialization.jsonObject(with: data) {
            var value = try MacrodexAgentJSONValue(jsonObject: object)
            if let purpose, case .object(var payload) = value {
                payload["purpose"] = .string(purpose)
                value = .object(payload)
            }
            return MacrodexAgentToolResult(callID: call.id, output: value)
        }

        if let purpose {
            return MacrodexAgentToolResult(callID: call.id, output: ["purpose": .string(purpose), "output": .string(output)])
        }
        return MacrodexAgentToolResult(callID: call.id, output: .string(output))
    }
    ...
    private func commandArguments(from args: [String: MacrodexAgentJSONValue]) -> [String] {
...
final class MacrodexAgentFoodSearchToolRunner: MacrodexAgentToolRunner {
    private let sqlRunner: MacrodexAgentSQLiteToolRunner

    init(databaseURL: URL) {
        self.sqlRunner = MacrodexAgentSQLiteToolRunner(databaseURL: databaseURL, maxRows: 25)
    }
    ...
    func runTool(_ call: MacrodexAgentToolCall) throws -> MacrodexAgentToolResult {
        ...
        let bindings = normalizedTokens.flatMap { token in [MacrodexAgentJSONValue.string(token), .string(token)] } + [.number(Double(limit))]
        ...
        var output: [String: MacrodexAgentJSONValue] = ["query": .string(query), "results": .array(rows)]
        if let purpose {
            output["purpose"] = .string(purpose)
        }
        return MacrodexAgentToolResult(callID: call.id, output: .object(output))
    }
}

enum MacrodexAgentMacrodexDatabaseToolError: Error, LocalizedError {
...
final class MacrodexAgentDatabaseSchemaToolRunner: MacrodexAgentToolRunner {
    private let sqlRunner: MacrodexAgentSQLiteToolRunner

    init(databaseURL: URL) {
        self.sqlRunner = MacrodexAgentSQLiteToolRunner(databaseURL: databaseURL)
    }
    ...
    func runTool(_ call: MacrodexAgentToolCall) throws -> MacrodexAgentToolResult {
        ...
        return MacrodexAgentToolResult(callID: call.id, output: output)
    }
}

final class MacrodexAgentDatabaseTransactionToolRunner: MacrodexAgentToolRunner {
    private let sqlRunner: MacrodexAgentSQLiteToolRunner

    init(databaseURL: URL) {
        self.sqlRunner = MacrodexAgentSQLiteToolRunner(
    ...
    func runTool(_ call: MacrodexAgentToolCall) throws -> MacrodexAgentToolResult {
        let args = call.arguments.objectValue ?? [:]
        do {
            let operations = try MacrodexAgentSQLiteToolRunner.transactionOperations(from: args["operations"])
...
final class MacrodexAgentLogFoodToolRunner: MacrodexAgentToolRunner {
    ...
    func runTool(_ call: MacrodexAgentToolCall) throws -> MacrodexAgentToolResult {
        ...
        do {
            ...
            let macros = Self.resolvedMacros(
                args: args,
                match: match,
                scale: scale,
                calories: calories,
            ...
            var operations: [MacrodexAgentSQLiteTransactionOperation] = []
            if shouldInsertEntry {
                operations.append(MacrodexAgentSQLiteTransactionOperation(
            ...
            operations.append(MacrodexAgentSQLiteTransactionOperation(
                ...
                bindings: [
                    .string(logItemId),
                    .string(entryId),
                    match?.canonicalFoodId.map(MacrodexAgentJSONValue.string) ?? .null,
                    match?.libraryItemId.map(MacrodexAgentJSONValue.string) ?? .null,
                    .string(logDate),
                    .number(Double(nowMs)),
                    .string(match?.name ?? foodName),
                    .number(serving.quantity),
                    .string(serving.unit),
                    serving.weightGrams.map(MacrodexAgentJSONValue.number) ?? .null,
                    .number(calories),
                    .number(protein),
                    .number(carbs),
                    .number(fat),
                    notes.map(MacrodexAgentJSONValue.string) ?? .null,
                    .number(Double(nowMs)),
                    .number(Double(nowMs))
                ],
                mode: .exec
            ...
            if case .object(var object) = output {
                ...
                object["serving"] = [
                    ...
                    "weightGrams": serving.weightGrams.map(MacrodexAgentJSONValue.number) ?? .null,
                ...
                if let match {
                    object["matchedFood"] = [
                        ...
                        "libraryItemId": match.libraryItemId.map(MacrodexAgentJSONValue.string) ?? .null,
                        "canonicalFoodId": match.canonicalFoodId.map(MacrodexAgentJSONValue.string) ?? .null,
                        "name": .string(match.name),
                        "brand": match.brand.map(MacrodexAgentJSONValue.string) ?? .null
                    ]
                }
                object["notes"] = .array(serving.notes.map(MacrodexAgentJSONValue.string))
                output = .object(object)
            }
            return MacrodexAgentToolResult(callID: call.id, output: output)
        } catch {
    ...
    private func resolveFood(args: [String: MacrodexAgentJSONValue], foodName: String) throws -> FoodMatch? {
    ...
    private func foodMatch(whereClause: String, bindings: [MacrodexAgentJSONValue], exactName: String? = nil) throws -> [FoodMatch] {
        let exactBinding = exactName.map(MacrodexAgentJSONValue.string) ?? .string("")
        let rows = try sqlRunner.query(
            """
    ...
    private func normalizedServing(args: [String: MacrodexAgentJSONValue], match: FoodMatch?) -> Serving {
    ...
    private static func foodNameFallback(args: [String: MacrodexAgentJSONValue]) -> String {
    ...
    private static func resolvedMacros(
        args: [String: MacrodexAgentJSONValue],
        match: FoodMatch?,
        scale: Double,
        calories: Double,
        foodName: String
    ) -> (protein: Double, carbs: Double, fat: Double) {
        let explicitProtein = args["protein"]?.numberValue ?? args["proteinG"]?.numberValue
        let explicitCarbs = args["carbs"]?.numberValue ?? args["carbsG"]?.numberValue
        let explicitFat = args["fat"]?.numberValue ?? args["fatG"]?.numberValue

        if let match {
            return (
                explicitProtein ?? match.protein * scale,
                explicitCarbs ?? match.carbs * scale,
                explicitFat ?? match.fat * scale
            )
        }

        guard calories > 0 else {
            return (explicitProtein ?? 0, explicitCarbs ?? 0, explicitFat ?? 0)
        }

        let explicitMacroCalories =
            (explicitProtein ?? 0) * 4
            + (explicitCarbs ?? 0) * 4
            + (explicitFat ?? 0) * 9
        let remainingCalories = max(calories - explicitMacroCalories, 0)
        let split = macroCalorieSplit(for: foodName)
        let missingProtein = explicitProtein == nil
        let missingCarbs = explicitCarbs == nil
        let missingFat = explicitFat == nil
        let missingWeight =
            (missingProtein ? split.protein : 0)
            + (missingCarbs ? split.carbs : 0)
            + (missingFat ? split.fat : 0)

        guard missingWeight > 0 else {
            return (explicitProtein ?? 0, explicitCarbs ?? 0, explicitFat ?? 0)
        }

        return (
            explicitProtein ?? (remainingCalories * split.protein / missingWeight / 4),
            explicitCarbs ?? (remainingCalories * split.carbs / missingWeight / 4),
            explicitFat ?? (remainingCalories * split.fat / missingWeight / 9)
        )
    }

    private static func macroCalorieSplit(for foodName: String) -> (protein: Double, carbs: Double, fat: Double) {
        let normalized = foodName.lowercased()
        if ["chicken", "turkey", "tuna", "fish", "salmon", "shrimp", "beef", "steak", "protein", "egg"]
            .contains(where: { normalized.contains($0) }) {
            return (0.55, 0.05, 0.40)
        }
        if ["rice", "bread", "pasta", "oat", "cereal", "potato", "banana", "apple", "fruit", "grape"]
            .contains(where: { normalized.contains($0) }) {
            return (0.10, 0.80, 0.10)
        }
        if ["cheese", "nut", "peanut", "almond", "avocado", "oil", "butter"]
            .contains(where: { normalized.contains($0) }) {
            return (0.15, 0.15, 0.70)
        }
        if ["milk", "yogurt", "smoothie", "shake"]
            .contains(where: { normalized.contains($0) }) {
    ...
    private static func firstString(_ values: MacrodexAgentJSONValue?...) -> String {
        for value in values {
    ...
    private static func failureResult(callID: String, message: String, hint: String? = nil) -> MacrodexAgentToolResult {
        var output: [String: MacrodexAgentJSONValue] = [
            "ok": false,
        ...
        return MacrodexAgentToolResult(callID: callID, output: .object(output), isError: true)
    }
...
final class MacrodexAgentSaveRecipeFromMealToolRunner: MacrodexAgentToolRunner {
    ...
    func runTool(_ call: MacrodexAgentToolCall) throws -> MacrodexAgentToolResult {
        ...
        guard !recipeName.isEmpty else {
            return Self.failureResult(callID: call.id, error: MacrodexAgentMacrodexDatabaseToolError.missingRecipeName)
        }
        ...
        do {
            ...
            guard !items.isEmpty else {
                return Self.failureResult(callID: call.id, error: MacrodexAgentMacrodexDatabaseToolError.missingRecipeItems)
            }
            ...
            guard !recipeId.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
                return Self.failureResult(callID: call.id, error: MacrodexAgentMacrodexDatabaseToolError.invalidRecipeId)
            }
            ...
            for (index, item) in items.enumerated() {
                ...
                operations.append(MacrodexAgentSQLiteTransactionOperation(
                    ...
                    bindings: [
                        ...
                        .string(recipeId),
                        componentItemId.map(MacrodexAgentJSONValue.string) ?? .null,
                        .string(item.name),
                        .number(item.quantity),
                        .string(item.unit),
                        item.weight.map(MacrodexAgentJSONValue.number) ?? .null,
            ...
            if case .object(var object) = transaction {
                object["purpose"] = purpose.map(MacrodexAgentJSONValue.string) ?? .string("Saving recipe")
                ...
                object["duplicate_recipe_ids"] = .array(duplicateRecipeIds.map(MacrodexAgentJSONValue.string))
                ...
                object["source"] = [
                    ...
                    "mealType": items.first?.mealType.map(MacrodexAgentJSONValue.string) ?? .null,
                ...
                object["components"] = .array(items.map { item in
                    ...
                    return [
                        ...
                        "componentItemId": componentItemId.map(MacrodexAgentJSONValue.string) ?? .null,
            ...
            return MacrodexAgentToolResult(callID: call.id, output: transaction)
        } catch {
            return Self.failureResult(callID: call.id, error: error)
        }
    }

    private func fetchMealItems(args: [String: MacrodexAgentJSONValue]) throws -> [MealItem] {
        ...
        var bindings: [MacrodexAgentJSONValue] = []
    ...
    fileprivate static func firstNonBlankString(_ values: MacrodexAgentJSONValue?...) -> String? {
        values.lazy
    ...
    fileprivate static func hasBlankString(_ values: MacrodexAgentJSONValue?...) -> Bool {
    ...
    ) -> [MacrodexAgentJSONValue] {
        var notes: [MacrodexAgentJSONValue] = []
...
final class MacrodexAgentFinalizeRecipeSaveToolRunner: MacrodexAgentToolRunner {
    private struct RecipeResolution {
        var id: String
        var creationOperation: MacrodexAgentSQLiteTransactionOperation?
    ...
    func runTool(_ call: MacrodexAgentToolCall) throws -> MacrodexAgentToolResult {
        ...
        let ignoredBlankRecipeId = MacrodexAgentSaveRecipeFromMealToolRunner.hasBlankString(args["recipeId"], args["canonicalRecipeId"], args["replaceExistingRecipeId"])
        ...
        do {
            ...
            guard !canonicalRecipeId.isEmpty else {
                return Self.failureResult(callID: call.id, error: MacrodexAgentMacrodexDatabaseToolError.missingCanonicalRecipe)
            }
            ...
            var transaction = try sqlRunner.transaction(operations, dryRun: dryRun)
            if case .object(var object) = transaction {
                object["purpose"] = purpose.map(MacrodexAgentJSONValue.string) ?? .string("Finalizing recipe")
                ...
                object["preview"] = .bool(dryRun)
                object["ignored_blank_recipe_id"] = .bool(ignoredBlankRecipeId)
                object["duplicate_recipe_ids"] = .array(recipeResolution.duplicateIds.map(MacrodexAgentJSONValue.string))
                object["notes"] = .array(MacrodexAgentSaveRecipeFromMealToolRunner.statusNotes(
                    ignoredBlankRecipeId: ignoredBlankRecipeId,
    ...
    private func canonicalRecipeResolution(args: [String: MacrodexAgentJSONValue], recipeName: String, now: Double) throws -> RecipeResolution {
        let requestedId = MacrodexAgentSaveRecipeFromMealToolRunner.firstNonBlankString(args["recipeId"], args["canonicalRecipeId"], args["replaceExistingRecipeId"])
        ...
        if let blank = try sqlRunner.query(
            ...
            return RecipeResolution(
                id: id,
                creationOperation: MacrodexAgentSQLiteTransactionOperation(
                    purpose: "Creating recipe",
                    statement: """
                    /* macrodex: Creating recipe */
                    INSERT INTO food_library_items (
        ...
        throw MacrodexAgentMacrodexDatabaseToolError.missingCanonicalRecipe
    }
    ...
    private static func logItemFilter(args: [String: MacrodexAgentJSONValue]) -> (sql: String, bindings: [MacrodexAgentJSONValue])? {
...
enum MacrodexAgentMacrodexToolDefinitions {
    static let healthKit = MacrodexAgentToolDefinition(
        name: "healthkit",
    ...
    static let foodSearch = MacrodexAgentToolDefinition(
        name: "food_search",
    ...
    static let logFood = MacrodexAgentToolDefinition(
        name: "log_food",
        description: "Fast-path log a single food item using Macrodex local food memory. It resolves a matching food, normalizes human serving units, writes the meal entry and food row atomically, and returns the inserted ids plus confirmation data in one tool call. Prefer this over raw SQL for normal calorie logging. If calories are supplied without full macros and no local match exists, Macrodex estimates missing protein/carbs/fat from the food type instead of storing all zeros. If it cannot find reliable local macros or blocks a zero-calorie real food, use web_search once and retry with explicit macros instead of logging bad data. Zero-calorie logs are allowed only for obvious zero-calorie foods or when confirmedZeroCalories is true after verification.",
        inputSchema: [
            "type": "object",
            "properties": [
                ...
                "calories": ["type": "number", "description": "Optional calories for this logged amount when no local match exists. Also provide or estimate macros when possible; missing macros are estimated as a fallback."],
    ...
    static let databaseSchema = MacrodexAgentToolDefinition(
        name: "db_schema",
    ...
    static let databaseTransaction = MacrodexAgentToolDefinition(
        name: "db_transaction",
    ...
    static let saveRecipeFromMeal = MacrodexAgentToolDefinition(
    ...
    static let finalizeRecipeSave = MacrodexAgentToolDefinition(
...
final class MacrodexAgentRuntimeBackend: AgentRuntimeBackend, @unchecked Sendable {
    static let shared = MacrodexAgentRuntimeBackend()

    let store: AppStore
    let client: AppClient
    let serverBridge: ServerBridge

    private let core: MacrodexAgentLocalRuntimeCore

    private init() {
        let core = MacrodexAgentLocalRuntimeCore()
        self.core = core
        self.store = MacrodexAgentAppStore(core: core)
        self.client = MacrodexAgentAppClient(core: core)
        self.serverBridge = MacrodexAgentServerBridge(core: core)
    }
    ...
    func startAsync() {
        Task.detached(priority: .userInitiated) { [core] in
            do {
                ...
                LLog.info("lifecycle", "Macrodex Agent runtime initialized")
            } catch {
                LLog.error("lifecycle", "Macrodex Agent runtime failed to initialize", error: error)
            }
        }
    }
...
private enum MacrodexAgentAppRuntimeError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .threadNotFound(let key):
            return "MacrodexAgent thread was not found: \(key.serverId)/\(key.threadId)"
        case .unsupported(let feature):
            return "\(feature) is not supported by the MacrodexAgent runtime adapter yet."
        case .noProvider:
            return "MacrodexAgent is not signed in to a model provider."
        case .subscriptionClosed:
            return "MacrodexAgent runtime update subscription closed."
        }
    }
}
...
private final class MacrodexAgentAppStore: AppStore, @unchecked Sendable {
    private let core: MacrodexAgentLocalRuntimeCore

    init(core: MacrodexAgentLocalRuntimeCore) {
        self.core = core
        super.init(noHandle: .init())
    }

    required init(unsafeFromHandle handle: UInt64) {
        fatalError("MacrodexAgentAppStore cannot be constructed from an external handle")
    }
    ...
    override func forkThreadFromMessage(key: ThreadKey, selectedTurnIndex: UInt32, params: AppForkThreadFromMessageRequest) async throws -> ThreadKey {
        throw MacrodexAgentAppRuntimeError.unsupported("Fork from message")
    }
    ...
    override func respondToApproval(requestId: String, decision: ApprovalDecisionValue) async throws {
        throw MacrodexAgentAppRuntimeError.unsupported("Approvals")
    }
    override func respondToUserInput(requestId: String, answers: [PendingUserInputAnswer]) async throws {
        throw MacrodexAgentAppRuntimeError.unsupported("User input prompts")
    }
    ...
    override func startReplay(data: String, targetKey: ThreadKey) async throws {
        throw MacrodexAgentAppRuntimeError.unsupported("Replay")
    }
    override func steerQueuedFollowUp(key: ThreadKey, previewId: String) async throws {}
    override func stopRecording() -> String { "{}" }
}

private final class MacrodexAgentAppClient: AppClient, @unchecked Sendable {
    private let core: MacrodexAgentLocalRuntimeCore
    ...
    required init(unsafeFromHandle handle: UInt64) {
        fatalError("MacrodexAgentAppClient cannot be constructed from an external handle")
    }
    ...
    override func execCommand(serverId: String, params: AppExecCommandRequest) async throws -> CommandExecResult {
        throw MacrodexAgentAppRuntimeError.unsupported("Shell commands")
    }

    override func startRemoteSshOauthLogin(serverId: String) async throws -> String {
        throw MacrodexAgentAppRuntimeError.unsupported("SSH OAuth")
    }
}

private final class MacrodexAgentServerBridge: ServerBridge, @unchecked Sendable {
    private let core: MacrodexAgentLocalRuntimeCore
    ...
    required init(unsafeFromHandle handle: UInt64) {
        fatalError("MacrodexAgentServerBridge cannot be constructed from an external handle")
    }
    ...
    override func connectRemoteServer(serverId: String, displayName: String, host: String, port: UInt16) async throws -> String {
        throw MacrodexAgentAppRuntimeError.unsupported("Remote servers")
    }

    override func connectRemoteUrlServer(serverId: String, displayName: String, websocketUrl: String) async throws -> String {
        throw MacrodexAgentAppRuntimeError.unsupported("Remote URL servers")
    }
}

private final class MacrodexAgentAppStoreSubscription: AppStoreSubscription, @unchecked Sendable {
    ...
    required init(unsafeFromHandle handle: UInt64) {
        fatalError("MacrodexAgentAppStoreSubscription cannot be constructed from an external handle")
    }

    override func nextUpdate() async throws -> AppStoreUpdateRecord {
        guard let update = await iterator.next() else {
            throw MacrodexAgentAppRuntimeError.subscriptionClosed
        }
        return update
    }
}

private final class MacrodexAgentLocalRuntimeCore: @unchecked Sendable {
    ...
    private struct PersistedReasoningState: Codable {
        var itemsByThreadID: [String: [PersistedReasoningItem]] = [:]
    }

    private struct PersistedReasoningItem: Codable {
        var id: String
    ...
    private var runtime: MacrodexAgentRuntime?
    private var toolDefinitions: [MacrodexAgentToolDefinition] = [
        ...
        MacrodexAgentMacrodexToolDefinitions.healthKit,
        MacrodexAgentMacrodexToolDefinitions.foodSearch,
        MacrodexAgentMacrodexToolDefinitions.logFood,
        MacrodexAgentMacrodexToolDefinitions.databaseSchema,
        MacrodexAgentMacrodexToolDefinitions.databaseTransaction,
        MacrodexAgentMacrodexToolDefinitions.saveRecipeFromMeal,
        MacrodexAgentMacrodexToolDefinitions.finalizeRecipeSave
    ]
    ...
    private var visibleModels: [ModelInfo] = MacrodexAgentBuiltInModelCatalogs.chatGPTCodex.visibleModels
        .map(MacrodexAgentLocalRuntimeCore.modelInfo)
    ...
    init() {
        ...
        let agentDirectory = appSupport.appendingPathComponent("MacrodexAgent", isDirectory: true)
        let legacyDirectory = appSupport.appendingPathComponent(Self.legacyRuntimeDirectoryName, isDirectory: true)
        ...
        Self.migrateLegacyStateIfNeeded(from: legacyDirectory, to: agentDirectory, fileManager: fileManager)
    }
    ...
    func subscribeUpdates() -> AppStoreSubscription {
        ...
        return MacrodexAgentAppStoreSubscription(stream: stream)
    }
    ...
    func renameThread(threadId: String, name: String) async throws {
        try await perform {
            guard var record = self.threads[threadId] else {
                throw MacrodexAgentAppRuntimeError.threadNotFound(ThreadKey(serverId: self.serverId, threadId: threadId))
            }
    ...
    func loginAccount(_ params: AppLoginAccountRequest) async throws {
        try await perform {
            ...
            case .chatgptAuthTokens(let accessToken, let chatgptAccountId, let chatgptPlanType):
                ...
                self.runtime?.registerProvider(provider, for: MacrodexAgentBuiltInProviderRegistry.openAI.id)
                ...
                self.emit(.serverChanged(serverId: self.serverId))
            case .chatgpt:
                throw MacrodexAgentAppRuntimeError.unsupported("Interactive ChatGPT login")
            case .apiKey:
                throw MacrodexAgentAppRuntimeError.unsupported("OpenAI API key provider")
            }
        }
    }
    ...
    private func ensureRuntimeSync() throws {
        ...
        let runtime = try MacrodexAgentRuntime(loadingStateFrom: stateFileURL)
        let registry = MacrodexAgentToolRegistry.defaultLocalTools(
            databaseURL: databaseURL,
            requiredSQLCommentMarker: "macrodex:"
        )
        registry.register(MacrodexAgentBuiltInToolDefinitions.title, runner: MacrodexAgentNoopTitleToolRunner())
        registry.register(
            MacrodexAgentMacrodexToolDefinitions.healthKit,
            runner: MacrodexAgentHealthKitToolRunner(workingDirectoryPath: defaultWorkingDirectoryPath)
        )
        registry.register(
            MacrodexAgentMacrodexToolDefinitions.foodSearch,
            runner: MacrodexAgentFoodSearchToolRunner(databaseURL: databaseURL)
        )
        registry.register(
            MacrodexAgentMacrodexToolDefinitions.logFood,
            runner: MacrodexAgentLogFoodToolRunner(databaseURL: databaseURL)
        )
        registry.register(
            MacrodexAgentMacrodexToolDefinitions.databaseSchema,
            runner: MacrodexAgentDatabaseSchemaToolRunner(databaseURL: databaseURL)
        )
        registry.register(
            MacrodexAgentMacrodexToolDefinitions.databaseTransaction,
            runner: MacrodexAgentDatabaseTransactionToolRunner(databaseURL: databaseURL)
        )
        registry.register(
            MacrodexAgentMacrodexToolDefinitions.saveRecipeFromMeal,
            runner: MacrodexAgentSaveRecipeFromMealToolRunner(databaseURL: databaseURL)
        )
        registry.register(
            MacrodexAgentMacrodexToolDefinitions.finalizeRecipeSave,
            runner: MacrodexAgentFinalizeRecipeSaveToolRunner(databaseURL: databaseURL)
        )
    ...
    private func tryInstallCodexAuthFileProviderSync() throws {
        ...
        let auth = try MacrodexAgentCodexAuth.loadFromCodexAuthFile(authFileURL)
        let provider = MacrodexAgentCodexChatGPTProvider(auth: auth)
        runtime?.registerProvider(provider, for: MacrodexAgentBuiltInProviderRegistry.openAI.id)
        if account == nil {
    ...
    private func tryInstallGoogleAIProviderSync() {
        guard let envKey = MacrodexAgentBuiltInProviderRegistry.google.authEnvironmentVariable,
              let apiKey = ProcessInfo.processInfo.environment[envKey]?.trimmingCharacters(in: .whitespacesAndNewlines),
              !apiKey.isEmpty
        else {
            return
        }
        runtime?.registerProvider(MacrodexAgentGoogleAIProvider(apiKey: apiKey), for: MacrodexAgentBuiltInProviderRegistry.google.id)
        if account == nil {
            authToken = nil
            authMode = .apiKey
            account = .apiKey
    ...
    private func availableModelInfos(includeHidden: Bool) -> [ModelInfo] {
        ...
        if isGoogleAIConfigured {
            models += runtime?.availableModels(
                ...
                includeHidden: includeHidden
            ) ?? MacrodexAgentBuiltInModelCatalogs.googleAI.visibleModels
        }
    ...
    private static func defaultReasoningEffortName(for model: String) -> String? {
        ...
        let effort = modelInfo.defaultReasoningEffort ?? modelInfo.supportedReasoningEfforts.first
        return effort == "none" ? nil : effort
    }

    private var isGoogleAIConfigured: Bool {
        guard let envKey = MacrodexAgentBuiltInProviderRegistry.google.authEnvironmentVariable else {
            return false
        }
    ...
    private func providerConfiguration(for model: String) -> MacrodexAgentProviderConfiguration {
        let providerID = Self.providerID(for: model)
        return MacrodexAgentProviderConfiguration(id: providerID, model: model)
    }
    ...
    private static func foodSearchNeedsWeb(
        ...
        if missingTokenCount == 0 && bestConfidence >= 0.94 {
            return false
        }
    ...
    private func rerankFoodSearchSync(query: String, candidates: [ComposerFoodSearchResult]) throws -> [ComposerFoodSearchResult] {
        ...
        let requestMessages = [
            ...
            MacrodexAgentMessage(role: .user, content: payloadJSON)
        ]

        var lastError: Error?
        var metadata: [String: MacrodexAgentJSONValue] = ["reasoning_effort": .string("low")]
        if UserDefaults.standard.bool(forKey: "fastMode") {
            metadata["service_tier"] = .string("fast")
        }
        for model in preferredModels {
            do {
                let response = try runtime.completeProvider(MacrodexAgentProviderRequest(
                    threadID: "food-search-rerank",
                    providerID: Self.providerID(for: model),
                    model: model,
                    messages: requestMessages,
                    tools: [MacrodexAgentBuiltInToolDefinitions.webSearch],
                    metadata: metadata
                ))
    ...
    private func dashboardFoodInsightsSync(payloadJSON: String) throws -> String {
        guard let runtime else { throw MacrodexAgentAppRuntimeError.noProvider }
        let availableModels = availableModelInfos(includeHidden: true).map(\.id)
        let preferredModels = [
            "gpt-5.4-mini",
            "gpt-5.4",
    ...
    private func scanNutritionLabelSync(imageData: Data) throws -> NutritionLabelScanResult {
        ...
        let messages = [
            MacrodexAgentMessage(role: .system, content: instructions),
            MacrodexAgentMessage(role: .user, content: "Scan this nutrition label.", imageURLs: [imageURI])
        ]
        var lastError: Error?
        for model in preferredModels {
            do {
    ...
    private func prepareTurnSync(key: ThreadKey, params: AppStartTurnRequest) throws -> PreparedTurn {
        ...
        let requestedEffort = params.effort.map(Self.reasoningEffortName)
        record.reasoningEffort = requestedEffort ?? record.reasoningEffort ?? Self.defaultReasoningEffortName(for: record.model)
        record.stats = Self.stats(for: record)
    ...
    private func runTurnSync(key: ThreadKey, params: AppStartTurnRequest, turnID: String) {
        do {
            guard let runtime else { throw MacrodexAgentAppRuntimeError.unsupported("MacrodexAgent runtime") }
    ...
    private func consumePendingSteerMessages(key: ThreadKey) -> [MacrodexAgentMessage] {
        lock.lock()
        guard var record = threads[key.threadId],
              record.activeTurnID != nil,
        ...
        emitThreadMetadata(record)
        return steers.flatMap { Self.agentMessages(from: $0.params.input) }
    }

    private func handleRuntimeEvent(_ event: MacrodexAgentRuntimeEvent, key: ThreadKey, turnID: String) {
        switch event.type {
        ...
        case "provider.reasoning_summary_text.delta":
            guard let delta = event.payload["delta"]?.stringValue, !delta.isEmpty else { return }
    ...
    private func appendReasoningDelta(
        ...
        if let index = record.items.firstIndex(where: { $0.id == itemID }) {
            var existing = record.items[index]
            if case .reasoning(var data) = existing.content {
                if data.summary.isEmpty {
                    data.summary = [delta]
                } else {
                    data.summary[data.summary.count - 1] += delta
                }
                existing.content = .reasoning(data)
            } else {
                existing.content = .reasoning(HydratedReasoningData(summary: [delta], content: []))
            }
            item = existing
        } else {
            item = HydratedConversationItem(
                id: itemID,
                content: .reasoning(HydratedReasoningData(summary: [delta], content: [])),
                sourceTurnId: turnID,
                sourceTurnIndex: nil,
                timestamp: Double(Self.nowMilliseconds()) / 1000.0,
                isFromUserTurnBoundary: false
        ...
        emit(.threadItemChanged(key: key, item: item, sessionSummary: sessionSummary(record)))
        emit(.threadStreamingDelta(key: key, itemId: itemID, kind: .reasoningText, text: delta))
    }

    private func reconcileReasoningText(
        _ text: String,
        itemID providerItemID: String?,
        key: ThreadKey,
        turnID: String
        ...
        if let index = record.items.firstIndex(where: { $0.id == itemID }) {
            var existing = record.items[index]
            existing.content = .reasoning(HydratedReasoningData(summary: [normalized], content: []))
            item = existing
        } else {
            item = HydratedConversationItem(
                id: itemID,
                content: .reasoning(HydratedReasoningData(summary: [normalized], content: [])),
                sourceTurnId: turnID,
                sourceTurnIndex: nil,
                timestamp: Double(Self.nowMilliseconds()) / 1000.0,
                isFromUserTurnBoundary: false
    ...
    private func upsertToolCall(
        ...
        var item: HydratedConversationItem
        let isWebSearch = name.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == MacrodexAgentBuiltInToolDefinitions.webSearch.name
        if let index = record.items.firstIndex(where: { $0.id == itemID }) {
            item = record.items[index]
            if isWebSearch {
        ...
        record.stats = Self.stats(for: record)
        threads[key.threadId] = record
        savePersistedReasoningItems(for: record)
        lock.unlock()
    ...
    private static func reasoningItemID(providerItemID: String?, turnID: String) -> String {
        let rawSuffix = providerItemID?
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .nilIfEmpty ?? "summary"
        let suffix = rawSuffix.map { character -> Character in
            character.isLetter || character.isNumber || character == "-" || character == "_" ? character : "-"
        }
        return "\(turnID)-reasoning-\(String(suffix))"
    }

    private func finalizeTurn(
        result: MacrodexAgentTurnResult,
        key: ThreadKey,
        turnID: String,
        inputMessages: [MacrodexAgentMessage]
    ) {
        var titleCandidate: String?
        var finalAssistantDelta: (itemID: String, text: String)?
        ...
        let toolSourceTurnIndices = Self.toolSourceTurnIndices(in: result.messages)
        let finalAssistantSourceTurnIndex = result.messages.lastIndex {
            $0.role == .assistant &&
        ...
        for message in currentTurnMessages {
            ...
            for call in calls {
                if call.name == MacrodexAgentBuiltInToolDefinitions.title.name,
        ...
        if let final = result.finalMessage, !final.content.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            ...
            if case .assistant(let data) = record.items.first(where: { $0.id == itemID })?.content {
                existingAssistantText = data.text
            } else {
                existingAssistantText = nil
            }
            let finalContent = Self.longestPrefixCompatibleAssistantText(
                existing: record.items.first(where: { $0.id == itemID }),
                finalContent: final.content
            )
            if let existingAssistantText,
               finalContent.count > existingAssistantText.count,
               finalContent.hasPrefix(existingAssistantText) {
                let suffix = String(finalContent.dropFirst(existingAssistantText.count))
                if !suffix.isEmpty {
                    finalAssistantDelta = (itemID, suffix)
                }
            }
            let item = HydratedConversationItem(
                id: itemID,
        ...
        if let finalAssistantSourceTurnIndex {
            let reasoningItems = record.items.filter { item in
                guard item.sourceTurnId == turnID else { return false }
                if case .reasoning = item.content { return true }
                return false
            }
            for var reasoningItem in reasoningItems {
                reasoningItem.sourceTurnIndex = finalAssistantSourceTurnIndex
                Self.upsertOrderedItem(reasoningItem, in: &record.items, fallbackBeforeItemID: record.activeAssistantItemID)
            }
        }
        ...
        if let finalAssistantDelta {
            emit(.threadStreamingDelta(
                key: key,
                itemId: finalAssistantDelta.itemID,
                kind: .assistantText,
                text: finalAssistantDelta.text
            ))
        }
    ...
    private static func longestPrefixCompatibleAssistantText(
        existing item: HydratedConversationItem?,
        finalContent: String
    ) -> String {
        guard case .assistant(let data) = item?.content else {
            return finalContent
        }
        let streamed = data.text
        if streamed.count > finalContent.count, streamed.hasPrefix(finalContent) {
            return streamed
        }
        if finalContent.count > streamed.count, finalContent.hasPrefix(streamed) {
            return finalContent
        }
        return finalContent
    }
    ...
    private func importRuntimeThreadSnapshotSync(_ thread: MacrodexAgentThreadSnapshot, includeMessages: Bool) throws {
    ...
    private func mergedToolDefinitions(dynamicTools: [AppDynamicToolSpec]) -> [MacrodexAgentToolDefinition] {
        ...
        for spec in dynamicTools where definitionsByName[spec.name] == nil {
            definitionsByName[spec.name] = MacrodexAgentToolDefinition(
                name: spec.name,
                description: spec.description,
                inputSchema: Self.agentJSONValue(fromJSONString: spec.inputSchemaJson) ?? .object([:])
            )
        }
    ...
    private static func agentMessages(from inputs: [AppUserInput]) -> [MacrodexAgentMessage] {
        ...
        return [MacrodexAgentMessage(role: .user, content: text, imageURLs: imageURLs)]
    }
    ...
    private static func messagesAfterCurrentInput(in messages: [MacrodexAgentMessage], inputMessages: [MacrodexAgentMessage]) -> [MacrodexAgentMessage] {
    ...
    private func loadPersistedReasoningItems(threadID: String) -> [HydratedConversationItem] {
        guard let data = try? Data(contentsOf: reasoningStateFileURL),
              let state = try? JSONDecoder().decode(PersistedReasoningState.self, from: data) else {
            return []
        }
        return (state.itemsByThreadID[threadID] ?? []).map { item in
            HydratedConversationItem(
                id: item.id,
                content: .reasoning(HydratedReasoningData(summary: item.summary, content: item.content)),
    ...
    private func savePersistedReasoningItems(for record: ThreadRecord) {
        let reasoningItems = record.items.compactMap { item -> PersistedReasoningItem? in
            guard case .reasoning(let data) = item.content else { return nil }
            let hasText = !(data.summary + data.content).allSatisfy {
                $0.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
            }
            guard hasText else { return nil }
            return PersistedReasoningItem(
                id: item.id,
                sourceTurnId: item.sourceTurnId,
                sourceTurnIndex: item.sourceTurnIndex,
                timestamp: item.timestamp,
        ...
        var state = PersistedReasoningState()
        if let data = try? Data(contentsOf: reasoningStateFileURL),
           let decoded = try? JSONDecoder().decode(PersistedReasoningState.self, from: data) {
            state = decoded
        }
        state.itemsByThreadID[record.key.threadId] = reasoningItems
        if let data = try? JSONEncoder().encode(state) {
            try? data.write(to: reasoningStateFileURL, options: [.atomic])
        }
    }
    ...
    private static func mergedConversationItems(
        ...
        guard !reasoningItems.isEmpty else { return items }
        var merged = items
        var existingIDs = Set(items.map(\.id))
        var fallbackAssistantSearchStart = merged.startIndex
        for item in reasoningItems.sorted(by: persistedReasoningSort) where !existingIDs.contains(item.id) {
            if item.sourceTurnIndex != nil {
                Self.upsertOrderedItem(item, in: &merged)
                if let insertedIndex = merged.firstIndex(where: { $0.id == item.id }) {
                    fallbackAssistantSearchStart = merged.index(after: insertedIndex)
                }
               ...
               let insertionIndex = merged.firstIndex(where: { existing in
                ...
                fallbackAssistantSearchStart = merged.index(after: insertionIndex)
            } else if let insertionIndex = nextAssistantInsertionIndex(in: merged, startingAt: fallbackAssistantSearchStart) {
                merged.insert(item, at: insertionIndex)
                fallbackAssistantSearchStart = merged.index(after: insertionIndex)
            } else {
                merged.append(item)
                fallbackAssistantSearchStart = merged.endIndex
            }
    ...
    private static func persistedReasoningSort(
        ...
        switch (lhs.sourceTurnIndex, rhs.sourceTurnIndex) {
    ...
    private static func nextAssistantInsertionIndex(
        in items: [HydratedConversationItem],
        startingAt startIndex: Int
    ) -> Int? {
        guard startIndex < items.endIndex else { return nil }
        return items[startIndex...].firstIndex {
            if case .assistant = $0.content { return true }
            return false
        }
    }

    private static func hydratedItems(from messages: [MacrodexAgentMessage], threadID: String) -> [HydratedConversationItem] {
        var items: [HydratedConversationItem] = []
        var toolCallsByID: [String: MacrodexAgentToolCall] = [:]
    ...
    private static func title(from messages: [MacrodexAgentMessage]) -> String? {
        for message in messages.reversed() {
            guard let calls = message.toolCalls else { continue }
            for call in calls.reversed() where call.name == MacrodexAgentBuiltInToolDefinitions.title.name {
    ...
    private static func preview(from messages: [MacrodexAgentMessage]) -> String? {
    ...
    private static func toolOutputsByCallID(_ messages: [MacrodexAgentMessage]) -> [String: String] {
    ...
    private static func toolSourceTurnIndices(in messages: [MacrodexAgentMessage]) -> [String: UInt32] {
        var indices: [String: UInt32] = [:]
        for (index, message) in messages.enumerated() {
            let sourceIndex = UInt32(index)
            if let calls = message.toolCalls {
                for call in calls {
                    indices[call.id] = sourceIndex
                }
            }
            if message.role == .tool, let callID = message.toolCallID {
                indices[callID] = sourceIndex
            }
        }
        return indices
    }

    private static func upsertOrderedItem(
        _ item: HydratedConversationItem,
        in items: inout [HydratedConversationItem],
        fallbackBeforeItemID: String? = nil
    ) {
        if let existingIndex = items.firstIndex(where: { $0.id == item.id }) {
            items.remove(at: existingIndex)
        }

        if let insertionIndex = orderedInsertionIndex(for: item, in: items) {
            items.insert(item, at: insertionIndex)
        } else if let fallbackBeforeItemID,
                  let fallbackIndex = items.firstIndex(where: { $0.id == fallbackBeforeItemID }) {
            items.insert(item, at: fallbackIndex)
        } else {
            items.append(item)
        }
    }
    ...
    private static func stats(for record: ThreadRecord) -> AppConversationStats {
        ...
        for item in record.items {
            ...
            case .dynamicToolCall(let data):
                dynamicToolCount += 1
                if data.tool == MacrodexAgentBuiltInToolDefinitions.webSearch.name {
                    webSearchCount += 1
                }
            case .webSearch:
                webSearchCount += 1
    ...
    private static func tokenUsage(_ usage: MacrodexAgentUsage) -> AppTokenUsage {
        AppTokenUsage(
    ...
    private static func modelInfo(_ model: MacrodexAgentModelInfo) -> ModelInfo {
    ...
    private static func agentJSONValue(fromJSONString json: String) -> MacrodexAgentJSONValue? {
        guard let data = json.data(using: .utf8),
              let object = try? JSONSerialization.jsonObject(with: data) else {
            return nil
        }
        return try? MacrodexAgentJSONValue(jsonObject: object)
    }

    private static func jsonString(_ value: MacrodexAgentJSONValue) -> String {
...
private struct MacrodexAgentNoopTitleToolRunner: MacrodexAgentToolRunner {
    func runTool(_ call: MacrodexAgentToolCall) throws -> MacrodexAgentToolResult {
        MacrodexAgentToolResult(
            callID: call.id,
            output: [
...
private extension MacrodexAgentMessage {
    func matchesTurnInput(_ input: MacrodexAgentMessage) -> Bool {
        role == input.role
...
private extension HydratedConversationItem {
    var isHydratedToolCallItem: Bool {
        switch content {
