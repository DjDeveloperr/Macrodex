...
final class AppModel {
    ...
    @ObservationIgnored private var runtimeBackend: (any AgentRuntimeBackend)?
    @ObservationIgnored private var storeBridge: AppStore
    @ObservationIgnored private var clientBridge: AppClient?
    @ObservationIgnored private var serverBridgeValue: ServerBridge?
    ...
    func resumeThread(
        ...
        // No overrides needed; refresh through the active local store.
    ...
    func restartLocalServer() async throws {
        await AgentRuntimeBootstrap.waitUntilReady()
    ...
    func ensureLocalServerConnected() async {
        let localServer = snapshot?.servers.first(where: \.isLocal)
        switch localServer?.health {
    ...
    func restoreStoredLocalChatGPTAuth(serverId: String) async {
        guard let storedTokens = (try? ChatGPTOAuthTokenStore.shared.load()) ?? nil else {
            return
        }

        let tokens: ChatGPTOAuthTokenBundle
        do {
            tokens = try await ChatGPTOAuth.refreshStoredTokens(previousAccountID: storedTokens.accountID)
        } catch {
            tokens = storedTokens
        }
    ...
    func preferredDefaultModelID(for serverId: String) -> String? {
        let models = availableModels(for: serverId)
        let configured = UserDefaults.standard.string(forKey: AppState.defaultChatModelKey)?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let preferredNames = [
            Self.nonBlank(configured),
            "gpt-5.4-mini",
            "gpt-5.4"
        ].compactMap { $0 }
        guard !models.isEmpty else { return preferredNames.first ?? "gpt-5.4-mini" }

        for preferredName in preferredNames {
            let normalizedPreferredName = preferredName.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
            if let match = models.first(where: { model in
                guard !isSparkModel(model) else { return false }
                let candidates = [
                    model.id,
                    model.model,
                    model.displayName
                ].map { $0.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() }
                return candidates.contains(normalizedPreferredName)
            }) {
                return match.id
            }
        }

        if let defaultModel = models.first(where: { $0.isDefault && !isSparkModel($0) }) {
            return defaultModel.id
        }
        return models.first(where: { !isSparkModel($0) })?.id ?? models.first?.id
    }
    ...
    func selectedModelID(
        ...
        if !trimmedSelection.isEmpty,
           !isSparkModelID(trimmedSelection),
           modelSupportsImages(trimmedSelection, in: models) {
            return trimmedSelection
        }

        let configuredImageModel = Self.nonBlank(
            UserDefaults.standard.string(forKey: AppState.defaultImageModelKey)?
                .trimmingCharacters(in: .whitespacesAndNewlines)
        )
        let preferredImageModels = ([configuredImageModel] + ["gpt-5.4", "gpt-5.4-mini", "gpt-5.5", "gpt-5.3-codex"])
            .compactMap { $0 }
        for preferredModel in preferredImageModels {
            if let match = models.first(where: { model in
                !isSparkModel(model) && modelMatches(model, preferredModel) && modelSupportsImages(model)
            }) {
                return match.id
            }
        }

        if let nonSparkImageModel = models.first(where: { model in
            !isSparkModel(model) && modelSupportsImages(model)
        }) {
            return nonSparkImageModel.id
        }

        // Metadata may still be loading on the first prompt. Prefer a known
        // vision-capable non-Spark model over the Spark default for image turns.
        return "gpt-5.4"
    }

    private func modelMatches(_ model: ModelInfo, _ identifier: String) -> Bool {
        let needle = identifier.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        let candidates = [model.id, model.model, model.displayName]
            .map { $0.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() }
    ...
    private func modelSupportsImages(_ modelID: String, in models: [ModelInfo]) -> Bool {
        guard let model = models.first(where: { modelMatches($0, modelID) }) else {
            return knownImageModelIDs.contains(modelID.trimmingCharacters(in: .whitespacesAndNewlines).lowercased())
        }
        return modelSupportsImages(model)
    }

    private func modelSupportsImages(_ model: ModelInfo) -> Bool {
        model.inputModalities.contains(.image)
            || knownImageModelIDs.contains(model.id.trimmingCharacters(in: .whitespacesAndNewlines).lowercased())
            || knownImageModelIDs.contains(model.model.trimmingCharacters(in: .whitespacesAndNewlines).lowercased())
    }

    private var knownImageModelIDs: Set<String> {
        ["gpt-5.5", "gpt-5.4", "gpt-5.4-mini", "gpt-5.3-codex"]
    }

    private func isSparkModel(_ model: ModelInfo) -> Bool {
        isSparkModelID(model.id) || isSparkModelID(model.model) || isSparkModelID(model.displayName)
    }

    private func isSparkModelID(_ modelID: String) -> Bool {
        modelID
            .trimmingCharacters(in: .whitespacesAndNewlines)
            .lowercased()
            .contains("spark")
