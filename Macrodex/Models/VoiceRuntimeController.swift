...
final class VoiceRuntimeController: VoiceActions {
    ...
    static let persistedLocalVoiceThreadIDKey = "macrodex.voice.local.thread_id"
    ...
    @ObservationIgnored private lazy var handoffManager = VoiceHandoffManager(localServerId: Self.localServerID)
    ...
    private func startRealtimeVoiceSession(
        ...
        let runtimeSessionId = "macrodex-voice-\(UUID().uuidString.lowercased())"
        ...
        activeVoiceSession = VoiceSessionState.initial(
            ...
            model: resolvedModel.isEmpty ? (model ?? "Assistant") : resolvedModel
        )
        do {
            var dynamicTools = AgentDynamicToolSpecs.defaultThreadTools(includeGenerativeUI: false) ?? []
            dynamicTools.append(contentsOf: try CrossServerTools.buildDynamicToolSpecs().map { try $0.rpcSpec() })
    ...
    private func realtimePrompt() -> String {
        VoiceSessionControl.buildPrompt()
    }
