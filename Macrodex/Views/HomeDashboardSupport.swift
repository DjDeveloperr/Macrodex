...
struct HomeDashboardRecentSession: Identifiable, Hashable {
    ...
    /// Tool activity log precomputed by the runtime snapshot. The iOS home
    /// card used to redo this walk client-side; using the snapshot log removes
    /// every `appModel.snapshot` read from the card at zoom 1-3.
    let recentToolLog: [AppToolLogEntry]
    /// Bounds of the most recent turn in milliseconds since epoch alongside
    /// `recent_tool_log`; we project into `Date` so
...
enum HomeDashboardSupport {
    ...
    static func sortedConnectedServers(
        ...
        return servers
            .filter { $0.isLocal && ($0.health != .disconnected || $0.connectionProgress != nil) }
            .map { server in
                HomeDashboardServer(
                    id: server.serverId,
    ...
    private static func sessionTitle(for session: AppSessionSummary) -> String {
        ...
        // The runtime `title` falls back to the literal "Untitled session" when
