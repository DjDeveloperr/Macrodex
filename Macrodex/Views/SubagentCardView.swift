...
struct SubagentCardView: View {
    ...
    private var headerRow: some View {
        ...
        .accessibilityHint("Shows agent work details")
    }
            ...
            HStack(alignment: .firstTextBaseline, spacing: 0) {
                ...
                if row.threadId != nil {
                    Button {
                        ...
                        Text("Open")
                            .macrodexFont(.caption)
                            .foregroundColor(resolvedKey != nil ? MacrodexTheme.textSecondary : MacrodexTheme.textMuted)
                    }
                    .buttonStyle(.plain)
                }
            }
                ...
                if let threadSnapshot {
                    ...
                    ScrollView {
                        if items.isEmpty {
                            VStack(spacing: 12) {
                                ...
                                Text(isLoading ? "Loading chat..." : "Waiting for agent output...")
                                    .macrodexFont(.caption)
                                    .foregroundColor(MacrodexTheme.textMuted)
                                Spacer()
                            }
                    ...
                    VStack(spacing: 12) {
                        ...
                        Image(systemName: "person.fill.questionmark")
                            .macrodexFont(size: 32)
                            .foregroundColor(MacrodexTheme.textMuted)
                        Text("Chat not available yet")
        ...
        VStack(spacing: 20) {
            SubagentCardView(
                data: ConversationMultiAgentActionData(
                    tool: "spawnAgent",
                    status: .inProgress,
                    prompt: "Explore /Users/dj/Developer/Macrodex with a repo-orientation focus. Scan the top-level directories and identify the main modules.",
