...
struct DirectoryPickerView: View {
    ...
    private var controls: some View {
        VStack(spacing: 8) {
            HStack(spacing: 8) {
                ...
                .foregroundColor(selectedServerOption == nil ? MacrodexTheme.textMuted : MacrodexTheme.textSecondary)
                .lineLimit(1)
                ...
                Button {
                    ...
                    Image(systemName: model.showHiddenDirectories ? "eye" : "eye.slash")
                        .foregroundColor(model.showHiddenDirectories ? MacrodexTheme.accent : MacrodexTheme.textSecondary)
                }
                .accessibilityLabel(
            ...
            .padding(.vertical, 8)
            .background(MacrodexTheme.surface.opacity(0.65))
            .overlay(
                ...
                HStack(spacing: 8) {
                    ...
                    ForEach(model.pathSegments()) { segment in
                        Button {
                            ...
                            Text(segment.label)
                                .macrodexFont(.caption)
                                .foregroundColor(segment.path == model.currentPath ? MacrodexTheme.textOnAccent : MacrodexTheme.textSecondary)
                                .padding(.horizontal, 10)
                                .padding(.vertical, 6)
                                .background(
                                    RoundedRectangle(cornerRadius: 8)
                                        .fill(segment.path == model.currentPath ? MacrodexTheme.accent : MacrodexTheme.surface.opacity(0.65))
                                )
                        }
                        .buttonStyle(.plain)
                    }
    ...
    private var content: some View {
        if model.isLoading {
            ProgressView().tint(MacrodexTheme.accent).frame(maxHeight: .infinity)
    ...
    private var bottomActionBar: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            HStack(spacing: 10) {
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 8)
                        .stroke(MacrodexTheme.border.opacity(0.75), lineWidth: 1)
                )
                .cornerRadius(8)
                ...
                .buttonStyle(.plain)
                .macrodexFont(.subheadline)
                .foregroundColor(canSelectPath ? MacrodexTheme.textOnAccent : MacrodexTheme.textMuted)
                .frame(maxWidth: .infinity)
                .padding(.vertical, 10)
                .background(canSelectPath ? MacrodexTheme.accent : MacrodexTheme.surface.opacity(0.65))
                .overlay(
                    RoundedRectangle(cornerRadius: 8)
                        .stroke(canSelectPath ? MacrodexTheme.accent.opacity(0.8) : MacrodexTheme.border.opacity(0.75), lineWidth: 1)
                )
                .cornerRadius(8)
            }
        }
...
#Preview("Directory Picker") {
    NavigationStack {
        ...
        .environment(MacrodexPreviewData.makeEmptyAppModel())
    }
}
#endif
