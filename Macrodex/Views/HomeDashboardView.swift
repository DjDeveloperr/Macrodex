...
struct HomeDashboardView: View {
    ...
    private var deleteConfirmationMessage: String {
        let title = deleteTargetThread?.sessionTitle ?? "this chat"
    ...
    private var topChrome: some View {
        ...
        .background(
            LinearGradient(
                colors: MacrodexTheme.headerScrim,
                startPoint: .top,
                endPoint: .bottom
            )
            .padding(.bottom, -30)
    ...
    private var bottomChrome: some View {
        ...
        .background(
            LinearGradient(
                colors: Array(MacrodexTheme.headerScrim.reversed()),
                startPoint: .top,
                endPoint: .bottom
            )
            .padding(.top, -30)
    ...
    private var emptyState: some View {
        VStack(spacing: 8) {
            ...
            Text(connectedServers.isEmpty
                 ? "Connect a server to start your first chat."
...
struct SessionCanvasLine: View {
    ...
    /// Derived from `recent_tool_log`, which records tool entries in
    /// chronological order — the last entry's status reflects
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                FormattedText(text: session.sessionTitle, lineLimit: zoomLevel >= 4 ? 4 : 1)
                    ...
                    .foregroundStyle(isActive ? MacrodexTheme.accent : MacrodexTheme.textPrimary)
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    Text(session.cwd)
                        ...
                        .foregroundStyle(MacrodexTheme.textMuted.opacity(0.7))
                        .lineLimit(2)
                        .padding(.top, 4)
                        .frame(maxWidth: .infinity, alignment: .leading)
        ...
        .background(alignment: .leading) {
            if isActive {
                MacrodexTheme.accent.opacity(0.3).frame(width: 2)
            }
        }
        .background(isActive ? MacrodexTheme.accent.opacity(0.02) : Color.clear)
        ...
        HStack(spacing: 4) {
            ...
            if isActive && isToolCallRunning {
                ...
                Text(session.serverDisplayName)
                    .foregroundStyle(MacrodexTheme.textSecondary.opacity(0.7))
    ...
    private var statChips: some View {
        ...
        if let tu = session.tokenUsage, let window = tu.contextWindow, window > 0 {
            ...
            Text("\u{00b7}")
                .foregroundStyle(MacrodexTheme.textMuted.opacity(0.5))
            Text("\(pct)%")
                .foregroundStyle(pct > 80 ? MacrodexTheme.warning.opacity(0.8) : MacrodexTheme.textMuted.opacity(0.8))
        }
    }
        ...
        HStack(spacing: 4) {
            ...
            HStack(spacing: 4) {
                ...
                Image(systemName: "server.rack")
                    .macrodexFont(size: 8)
                    .foregroundStyle(MacrodexTheme.accent.opacity(0.5))
                Text(session.serverDisplayName)
                    .foregroundStyle(MacrodexTheme.accent.opacity(0.6))
                let m = session.model.trimmingCharacters(in: .whitespacesAndNewlines)
                if !m.isEmpty {
                    Text("\u{00b7}").foregroundStyle(MacrodexTheme.textMuted.opacity(0.5))
                    Text(m)
                        .foregroundStyle(MacrodexTheme.textSecondary.opacity(0.7))
                }
                if session.isFork {
                    Text("\u{00b7}").foregroundStyle(MacrodexTheme.textMuted.opacity(0.5))
                    Text("fork")
                        .foregroundStyle(MacrodexTheme.warning.opacity(0.8))
                }
    ...
    private var inlineStats: some View {
        HStack(spacing: 6) {
            ...
            if let stats = s, stats.diffAdditions > 0 || stats.diffDeletions > 0 {
                HStack(spacing: 2) {
                    ...
                    Text("-\(stats.diffDeletions)")
                        .foregroundStyle(MacrodexTheme.danger.opacity(0.6))
                }
            }
            // Turn stopwatch reads pre-derived bounds from the session summary,
            // so the chip stays prop-driven.
            ...
            if let tu = session.tokenUsage, let window = tu.contextWindow, window > 0 {
                let pct = Int((Double(tu.totalTokens) / Double(window)) * 100)
                Text("\(pct)%")
                    .foregroundStyle(pct > 80 ? MacrodexTheme.warning.opacity(0.8) : MacrodexTheme.textMuted.opacity(0.7))
            }
        }
    }
        ...
        if !message.isEmpty && message != title {
            HStack(alignment: .top, spacing: 6) {
                ...
                FormattedText(text: message, lineLimit: 1)
                    .foregroundStyle(MacrodexTheme.textSecondary.opacity(0.9))
            }
            // Match the conversation view's user-message size
            // (`UserBubble` uses `.macrodexFont(size: conversationBodyPointSize)`).
    ...
    private func toolLog(maxEntries: Int) -> some View {
        // `recent_tool_log` already derives this; the iOS copy that used to
        // live here was the dominant AttributeGraph subscription during
        // streaming. Entries come through newest-last; take the tail to show
        // the most recent `maxEntries`.
    ...
    private func toolRowView(_ entry: AppToolLogEntry) -> some View {
        ...
        // `ToolCallCardView` uses `.macrodexFont(size: contentFontSize)` =
        // `MacrodexFont.conversationBodyPointSize` — and a regular (non-mono)
        // font so titles/messages/tools all read at the same scale.
        .macrodexFont(size: MacrodexFont.conversationBodyPointSize)
        .frame(maxWidth: .infinity, alignment: .leading)
    }

    /// Pick the display glyph/icon for a tool-log entry. `AppToolLogEntry.tool`
    /// is a short category name (`"Bash"`, `"Edit"`, `"MCP"`,
    ...
    private func toolIconView(for tool: String) -> some View {
        ...
        case "Tool":
            Image(systemName: "wrench.and.screwdriver")
                .macrodexFont(size: 12, weight: .semibold)
        case "Bash":
            Text("$").macrodexFont(size: 12, weight: .semibold)
        case "Edit":
            Text("✎").macrodexFont(size: 12, weight: .semibold)
    ...
    private var responsePreview: some View {
        ...
        // walking `hydratedConversationItems` live. The runtime refreshes
        // the session summary on every item delta, but the home
        // dashboard's observation path is debounced at 120ms in
        // `HomeDashboardModel.scheduleObservedRefresh` — so the preview
        // naturally updates at ~8Hz instead of forcing `MacrodexMarkdownView`
...
/// message bodies (`MacrodexFont.conversationBodyPointSize × textScale`) so
...
private struct MarkdownMatchedTitleFont: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            .font(.custom(
                MacrodexFont.markdownFontName,
                size: MacrodexFont.conversationBodyPointSize * textScale
            ))
            .fontWeight(.medium)
    }
}
