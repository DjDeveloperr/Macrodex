...
final class HomeRowContainer: UIView {
    ...
    init(scrollHost: HomeSessionsScrollUIView) {
        ...
        pinchHighlight.backgroundColor = UIColor(MacrodexTheme.accent).withAlphaComponent(0.14)
    ...
    private func updateActionsVisuals() {
        ...
        if offsetX > 0 {
            actionsBackground.backgroundColor = UIColor(MacrodexTheme.accent)
            ...
            actionsBackground.backgroundColor = UIColor(MacrodexTheme.danger)
            actionsBackground.alpha = tintAlpha
...
private struct SessionContextMenuPreview: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text(session.sessionTitle.isEmpty ? "Chat" : session.sessionTitle)
                .macrodexFont(size: MacrodexFont.conversationBodyPointSize, weight: .medium)
            ...
            if !session.serverDisplayName.isEmpty {
                Text(session.serverDisplayName)
                    ...
                    .foregroundStyle(MacrodexTheme.textSecondary.opacity(0.75))
                    .lineLimit(1)
            }
        }
