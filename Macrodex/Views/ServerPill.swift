    ...
    var body: some View {
        Button(action: onTap) {
            HStack(spacing: 6) {
                ...
                Text(server.displayName)
                    .macrodexMonoFont(size: 13, weight: .semibold)
        ...
        .overlay(
            Capsule(style: .continuous)
                .stroke(
                    isSelected ? MacrodexTheme.accent.opacity(0.75) : MacrodexTheme.textMuted.opacity(0.25),
...
struct AddServerPill: View {
    ...
    var body: some View {
        ...
        .overlay(
            Capsule(style: .continuous)
                .stroke(MacrodexTheme.accent.opacity(0.45), lineWidth: 0.8)
                .allowsHitTesting(false)
        )
    }
}
