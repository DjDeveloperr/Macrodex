...
struct RealtimeVoiceScreen: View {
    ...
    private var primaryTextColor: Color {
        MacrodexTheme.textPrimary
    }

    private var secondaryTextColor: Color {
        MacrodexTheme.textSecondary
    }

    private var promptFillColor: Color {
        MacrodexTheme.surface.opacity(colorScheme == .dark ? 0.82 : 0.92)
    }

    private var promptStrokeColor: Color {
        MacrodexTheme.border.opacity(colorScheme == .dark ? 0.55 : 0.8)
    }

    private var controlFillColor: Color {
        MacrodexTheme.surfaceLight.opacity(colorScheme == .dark ? 0.72 : 0.88)
    }
    ...
    private var visibleTranscriptEntries: [VoiceSessionTranscriptEntry] {
        ...
        let speaker = session.transcriptSpeaker?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false
            ...
            : (session.phase == .speaking ? "Assistant" : "You")
        ...
        VStack(spacing: 16) {
            HStack(spacing: 8) {
                ...
                Text(phase.displayTitle.uppercased())
                    .font(MacrodexFont.monospaced(.caption, weight: .bold))
    ...
    private func transcriptLine(
        ...
        return Text(entry.text)
            .font(MacrodexFont.styled(textStyle, weight: fontWeight))
        ...
        HStack(spacing: 40) {
                ...
                Button(action: onToggleSpeaker) {
                    VStack(spacing: 6) {
                        ...
                        Text(session.route.label)
                            .font(MacrodexFont.monospaced(.caption2, weight: .medium))
                    }
                    .foregroundColor(
            ...
            Button(action: onEnd) {
                Image(systemName: "xmark")
                    ...
                    .background(MacrodexTheme.danger)
                    .clipShape(Circle())
            }
            .buttonStyle(.plain)
        }
    ...
    private var realtimeApiKeyPrompt: some View {
        VStack(alignment: .leading, spacing: 14) {
            Text("Realtime needs an API key")
                .font(MacrodexFont.styled(.headline, weight: .semibold))
                .foregroundColor(primaryTextColor)

            Text("Enter your OpenAI API key for this device. Macrodex will store it in the local agent environment as OPENAI_API_KEY.")
                .font(MacrodexFont.styled(.caption))
            ...
            SecureField("sk-...", text: $apiKey)
                ...
                .font(MacrodexFont.monospaced(.body))
                .foregroundColor(primaryTextColor)
    ...
    private var apiKeySaveButtonLabel: some View {
        HStack(spacing: 10) {
            ...
            Text(isSavingApiKey ? "Saving…" : "Save API Key")
                .font(MacrodexFont.styled(.subheadline, weight: .semibold))
        }
        .foregroundColor(primaryTextColor)
        .frame(maxWidth: .infinity)
        .padding(.vertical, 12)
